...
import com.omalik.android.hockeydj.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        SpotifyService.connectionInterface = this
        SpotifyService.activity = this
        spotify.context = this
        Log.d(TAG, "Spotify authentication...")
    ...
    override fun connectionError(msg: String?, throwable: Throwable) {
        ...
        var message = msg
        if (msg == null) {
            message="No error message received. The Spotify app seems not to be installed"
        }
        val errorDialog = ConnectionErrorDialog()
        val bundle = bundleOf(
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, intent: Intent?) {
        ...
        when(requestCode) {
            ...
            SpotifyService.REQUEST_CODE -> {
                SpotifyService.onActivityResult(requestCode, resultCode, intent)
            }
    ...
    fun scaleBitmap(bitmap: Bitmap) : Bitmap {
        ...
        if (bitmap.width > 2 * displayMetrics.widthPixels) {
            ...
            Log.d(TAG, "ScaledBitmap: ${scaledBitmap.width}x${scaledBitmap.height}")
            return scaledBitmap
        } else {
            Log.d(TAG, "Bitmap: ${bitmap.width}x${bitmap.height}")
            return bitmap
